...
static QStringList stringListKeywords = QStringList() << "FORMS" << "HEADERS" << "SOURCES" << "TRANSLATIONS" << "TEXTS" << "SUBDIRS" << "OPENEDFILES";
//
...
QMakeProject::QMakeProject( const QString& filePath, QTreeWidget* tree, QObject* parent )
	...
	mTree( tree ), mOpen( false ), mModified( false ), mLocked( false )
{
	mSubProjects.clear();
	stringValues.clear();
...
bool QMakeProject::save( bool locked )
{
	...
	if ( f.open( QFile::ReadOnly | QFile::Text ) )
	{
		...
		if ( !contents.contains( _PROGRAM_NAME ) )
			contents.prepend( QString( "message( \"This file has been generated by %1 (%2) - %3\" )\n\n" ).arg( _PROGRAM_NAME ).arg( _PROGRAM_VERSION ).arg( _PROGRAM_WEBSITE ) );
