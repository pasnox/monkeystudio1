...
// this plugin view your variable value
// it can modifit this
/************************************************************************/
...
QIcon Watch::pluginMenuIcon()
{
	...
	return  QIcon( ":/Icons/Icons/explorer.png" );
}


void eraseAllChild(QTreeWidgetItem *id )
{
	for(int j=0; j< id->childCount(); j++)
	{
		id->setTextColor(3, Qt::black); 
		if(id->child(j)->childCount()) eraseAllChild(id->child(j));
...
QDialog * Watch::pluginSetting()
{
	return settingWatch::self();
}
