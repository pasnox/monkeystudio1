...
void UIMain::addTab( MDIChild* child )
{
	...
	if ( !exists )
	{
		...
		// xiantia, connect message emit by editor (CTRL + B) to function
		connect( child, SIGNAL(signalBreakpointAt(QString, int)), this, SLOT(on_teToggleBreakpoint(QString,int)));
		// end
...
void UIMain::removeTab( MDIChild* child )
{
	...
	for ( int i = 0; i < tabBar->count(); i++ )
	{
		if ( tabBar->tabText(i) == caption && tabBar->tabData( i ).toString() == fileName )
		{
			// xiantia, disconnect message, tab removed
			disconnect( child, SIGNAL(signalBreakpointAt(QString, int)), this, SLOT(on_teToggleBreakpoint(QString,int)));
			// end
...
// debuger message CTRL + B from editor
...
bool haveFile =false;
	...
	// memorise l'encien editor qui avait la fleche bleu
	if ( mLastEditor )
		mLastEditor->gotoDebugerLine("" /*mLastEditor->filePath()*/, 0, false );

	if ( !mProject )
                		return;

	// on prend le nom du fichier et on cherche dans tt les fichier pr retrouver son chemin absolu
	QString mFileName = QFileInfo( s ).fileName();
	foreach ( QString k, mProject->cStringListValues().keys() ) // SOURCES, HEADERS, FORMS ...
	{
    		foreach ( QString p, mProject->cStringListValues()[ k ].keys() ) // (all), win32, unix, mac
		{ 
			foreach ( QString v, mProject->cStringListValues()[ k ][ p ] ) // stringlist values
                    		{
                    			if ( QFileInfo( v ).fileName() == mFileName )
				{
			                                        mFileName = mProject->absolutePath().append( QString( "/%1" ).arg( v ) );
					// on recherche si header ou cpp et on ouvre
					if ( QMakeProject::headerExtension.contains( QFileInfo( mFileName ).suffix() ) )
					{
						// ouvre la classe
						createWindow( mFileName, pHeader, mProject );
						// on recuperel a fenetre
						MDIClass* m = qobject_cast<MDIClass*>( workspace->activeWindow() );
						if ( !m )
							return;
						// on va a la bonne ligne
						haveFile = true;
						m->teHeader->gotoDebugerLine( mFileName, i, true );
						mLastEditor = m->teHeader;
					}
					else
					{
						// ouvre la classe
						createWindow( mFileName, pSource, mProject );
						// on recuperel a fenetre
						MDIClass* m = qobject_cast<MDIClass*>( workspace->activeWindow() );
						if ( !m )
							return;
						// on va a la bonne ligne
						haveFile =true;
						m->teSource->gotoDebugerLine( mFileName, i, true );
						mLastEditor = m->teSource;
					}
				}
			}
		}
	...
	if(!haveFile && !s.isEmpty()) // le fishier n'a pas etait trouv�
	{
